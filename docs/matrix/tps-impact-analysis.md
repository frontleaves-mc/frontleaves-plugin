...
- **时钟**：使用 `time.Ticker` 系统时钟，manageInterval 默认为 5s（可通过 `MATRIX_MANAGE_INTERVAL` 环境变量配置），与 TPS 无关
...
| 参数 | 值 | 来源 | 可配置 |
|---|---|---|---|
| speedThreshold | 12.0 blocks/s | `sub_anti_cheat.go` 第 20 行 | 否 |
| manageInterval | 5s（默认） | `player_session.go` init()（L29-34） | 是（MATRIX_MANAGE_INTERVAL） |
| popBatchSize | 100 条/批 | `player_session.go` 第 24 行 | 否 |
...
Matrix 的 manageLoop 以 5s（默认，可通过 `MATRIX_MANAGE_INTERVAL` 环境变量配置）为周期从 Redis 批量消费数据（popBatchSize=100）。这个周期与 TPS 完全无关，由 `time.Ticker` 驱动。

- TPS=20 时：每个 5s manage 周期内约有 100 个 TelemetryTick 被处理（20 tick/s × 5s = 100）
- TPS=10 时：每个 5s manage 周期内约有 50 个 TelemetryTick 被处理（10 tick/s × 5s = 50）
- TPS=5 时：每个 5s manage 周期内约有 25 个 TelemetryTick 被处理（5 tick/s × 5s = 25）
...
3. Reach 检测（攻击距离）不受 Δtime 影响，完全不受 TPS 波动干扰，已在代码中实现（`sub_anti_cheat.go` checkReach）
...
4. **manageInterval 可配置性**：当前 manageInterval 默认为 5s，可通过环境变量 `MATRIX_MANAGE_INTERVAL` 配置。如果未来改为动态调整，需重新评估。
...
| manageInterval | 5s（默认） | `player_session.go` init()（可通过 MATRIX_MANAGE_INTERVAL 环境变量配置） |
